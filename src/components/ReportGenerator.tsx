...
export function ReportGenerator({ theme }: ReportGeneratorProps) {
  ...
  const [endHour, setEndHour] = useState<string>('');
      ...
      const handleGenerateReport = async () => {
        ...
        try {
          setGenerating(true);
          console.log(`Buscando citas para: ${selectedProf} en rango ${startDate} a ${endDate} (Horas: ${startHour || 'Todas'} - ${endHour || 'Todas'})`);
          ...
          while (hasMore) {
            ...
            if (data && data.length > 0) {
              ...
              // Aplicar filtro de horas localmente
              if (startHour || endHour) {
                filteredData = data.filter((row: any) => {
                  if (!row.turno) return false;
                  const timePart = row.turno.split('T')[1]?.substring(0, 5); // ej. '14:30'
                  if (!timePart) return false;
                  
                  if (startHour && endHour && startHour > endHour) {
                    // Lógica para turno nocturno (ej. de 20:00 a 08:00)
                    // Es inválido si está en el hueco que no corresponde al turno (ej. de 08:01 a 19:59)
                    if (timePart < startHour && timePart > endHour) return false;
                  } else {
                    // Lógica normal (ej. de 08:00 a 16:00)
                    if (startHour && timePart < startHour) return false;
                    if (endHour && timePart > endHour) return false;
                  }
  ...
  return (
    <div className={`flex-1 w-full ${theme.cardBg} rounded-2xl border ${theme.cardBorder} shadow-xl flex flex-col p-6 animate-fade-in`}>
      ...
      {/* Filter Row */}
      <div className="grid grid-cols-1 md:grid-cols-6 gap-4 mb-6 bg-slate-900/45 p-4 rounded-2xl border border-slate-800/80">
        {/* Professional Select */}
        ...
        {/* Start Hour */}
        <div className="flex flex-col gap-1.5 col-span-1 md:col-span-1">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Hora Desde</label>
          <input
            type="time"
            value={startHour}
        ...
        {/* End Hour */}
        <div className="flex flex-col gap-1.5 col-span-1 md:col-span-1">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Hora Hasta</label>
          <input
            type="time"
            value={endHour}
        ...
        {/* Action Buttons */}
        <div className="flex items-end gap-2 col-span-1 md:col-span-6 justify-end">
          <button
